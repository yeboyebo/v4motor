...
*/
	...
	*/
		function showItem(i,speed,callback) {
			...
			if(opts.show_filmstrip) {
				...
				movGallery(j_frames.eq(i).addClass('current').find('img').attr('pos'));
	...
	*/
	...
	// If user clicks the start/stop button
	$('#start_stop').click(function(){
		if(!paused || $('#start_stop').attr('paused')=='false') {
			// Pause slideshow in 500ms
			$(document).oneTime(500,"animation_pause",function(){
			$(document).stopTime("transition");
				paused=true;
				$('#start_stop').attr('paused','true');
			});
			$("#start_stop").fadeOut(100,function(){
				$(this).css('background','url(../images/btn-play.png) no-repeat');
				$(this).fadeIn(200);
			});
		}else{
			...
			if(paused) {
				...
				paused = false;
				$('#start_stop').attr('paused','false');
				$("#start_stop").fadeOut(100,function(){
					$(this).css('background','url(../images/btn-pause.png) no-repeat');
					$(this).fadeIn(200);
				});
			}
		}
